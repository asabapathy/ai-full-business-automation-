...
resource "aws_vpc" "main" {
  ...
  tags = { Name = "${var.project}-${var.environment}-vpc", Environment = var.environment }
}
...
resource "aws_subnet" "public" {
  ...
  tags = { Name = "${var.project}-${var.environment}-public-${count.index + 1}", "kubernetes.io/role/elb" = "1" }
}
...
resource "aws_subnet" "private" {
  ...
  availability_zone = var.availability_zones[count.index]
  tags = { Name = "${var.project}-${var.environment}-private-${count.index + 1}", "kubernetes.io/role/internal-elb" = "1" }
}
